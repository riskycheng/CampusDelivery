...
//		getActivity().getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);  //设置全屏
	...
	/*	
		if(ConstantParams.isFromLoginActivity || ConstantParams.isFromSplashActivity) {	//从登录界面，或者从SplashActivity跳转过来的
	...
	*/	
		...
		companySpinner.setOnItemSelectedListener(new OnItemSelectedListener(){
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				else if(companyManuSelect.equals("圆通快递")) {
					companyId = 101;
				}
				else if(companyManuSelect.equals("韵达快运")) {
					companyId = 102;
				}
				...
				else if(companyManuSelect.equals("中通速递")) {
					companyId = 500;
				}
				else if(companyManuSelect.equals("天天快递")) {
					companyId = 504;
				}
				else if(companyManuSelect.equals("顺丰快递")) {
					companyId = 505;
				}
	            if(companyId != -1) {
									...
									public void onClick(DialogInterface dialog, int which) {
										...
										if(!mailNoStr.equals("")) {
											if(isLetterOrDigit(mailNoStr)) {
												...
												Toast.makeText(mActivity,"运单号只能包含字母或数字，请重新输入！",Toast.LENGTH_SHORT).show();
											}
										}
										else {
											mailNo.setText(mailNoStr);
...
private void billInStore() {
	...
	if(mailNo.equals("") || receiverNameStr.equals("") || receiverMobilephoneStr.equals("")) {
		...
		Toast.makeText(mActivity, "无效物流公司，请手动选择！", Toast.LENGTH_SHORT).show();
	}
	...
	if(requestCode == 1) {	//入库操作
		if(resultCode == Activity.RESULT_OK) {
			...
			if(result.equals("查询失败") || result.equals("查询为空")) {
				...
				Toast.makeText(mActivity, mailNoStr + " 入库成功！", Toast.LENGTH_SHORT).show();
				new Thread() {
					@Override
					public void run() {
						try {
...
//							handler.sendEmptyMessage(2);
							...
							e.printStackTrace();
							Toast.makeText(mActivity, "HTTP访问异常！", Toast.LENGTH_SHORT).show();
						}
//						finally {
//							try {
...
//						}
		...
		if(resultCode == Activity.RESULT_OK) {
		    ...
		    try {
		        ...
		        Toast.makeText(mActivity, "快件查询成功", Toast.LENGTH_SHORT).show();
		    } catch (JSONException e) {	//快件查询异常
		        Toast.makeText(mActivity, "快件不存在", Toast.LENGTH_SHORT).show();
				companyId = mySharedPreferences.getInt("prev_company_id", -1);	//获取上一次的快递公司
            ...
            receiverMobilephone.setSelection(receiverMobilephone.getText().toString().length());	//设置光标位置
		}
	}
		...
		if(resultCode == Activity.RESULT_OK) {
			...
			if(result.equals("查询失败") || result.equals("查询为空")) {
				...
				Toast.makeText(mActivity, "查询物流公司列表成功！", Toast.LENGTH_SHORT).show();
				try {
	...
	public void handleMessage(Message msg) {
		...
		case HadwareControll.BARCODE_READ:
			...
			if(!mailNoStr.equals("") && isLetterOrDigit(mailNoStr)) {
				...
				Toast.makeText(mActivity,"运单号有误，请重新扫描！",Toast.LENGTH_SHORT).show();
			}
			break;
		default:
			break;
