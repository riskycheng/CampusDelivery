...
public class LoginActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    copyright.setText("Copyright ©南京宇牧信息科技有限公司 2008-" + i + " All Rights Reserved.");
...
//		editor.commit();
	...
	private void login() {
		...
		if(usernameUserInput.equals("") && passwordUserInput.equals("")) {
		...
		else if(usernameUserInput.equals("") && !passwordUserInput.equals("")) {
			...
			//查询管理员id、密码
			intent.putExtra("SQL", "select id, adm_password from 020_admin where adm_name=\"" + usernameUserInput + "\"");
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode == RESULT_OK) {
			...
			if(result.equals("查询失败")) {
				...
				String jsonString = result.substring(1, result.length()-1);		//去掉两头的中括号
				try {
					JSONObject jsonObject = new JSONObject(jsonString);		//转换为JSONObject
...
//				if(passwordUserInput.equals(password)) {
				if(password2MD5.equals(password)) {
					...
					String loginTimeStore = mySharedPreferences.getString("login_time","2014-01-01 00:00:00");
			        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");   //获取当前时间
			        String loginTime = simpleDateFormat.format(new java.util.Date());
			        if(!loginTime.substring(0, 10).equals(loginTimeStore.substring(0, 10))) {
			        	...
			        	String currentInStoreBillNo = new SimpleDateFormat("yyyyMMdd").format(new java.util.Date()) + "-" + "1";
